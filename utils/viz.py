...
def prot_to_graph(prot, cutoff):
    ...
    g.ndata["pos"] = th.tensor(np.array(
...
def mol_to_graph2(prot_path, lig_path, cutoff, explicit_H=False, use_chirality=True):
...
def pdbbind_handle(pdbid, args):
    prot_path = "%s/%s/pro-%s_pocket_%s.pdb" % (args.dir, pdbid, pdbid,args.cutoff)
...
def UserInput():
    ...
    p.add_argument('-o', '--outprefix', default="10A",
...
def main():
    ...
    if args.parallel:
        results = Parallel(n_jobs=8)(delayed(pdbbind_handle)(pdbid, args) for pdbid in pdbids)
    else:
        results = []
        for index,pdbid in enumerate(pdbids):
            print(index)
