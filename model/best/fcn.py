...
#DTIPredictorV4 - 主要预测模型
# DTIPredictorV4Cat: 添加全局特征拼接
# DTIPredictorV4VS: 虚拟筛选专用版本
# DTIPredictorV4_V2: 改进版本，处理不同的输入特征
# DTIPredictorV4_V2_InTra_Inter: 同时考虑分子内和分子间相互作用
# DTIPredictorV4_V2_Mask: 支持掩码操作
# DTIPredictorV4_V2_Test: 测试版本
...
class DTIPredictorV4_V2(nn.Module):
    def __init__(self, node_feat_size, edge_feat_size, num_layers, graph_feat_size, outdim_g3,
        ...
        # graph layers for ligand and protein
        self.cov_graph = ModifiedAttentiveFPPredictorV2(node_feat_size=41, edge_feat_size=6, num_layers=3, graph_feat_size=200, dropout=0)
    ...
    def forward(self, bg, bg3):
        atom_feats = bg.ndata.pop('atom')
        bond_feats = bg.edata.pop('bond')
        atom_feats = self.cov_graph(bg, atom_feats, bond_feats)
        bond_feats3 = bg3.edata['feats']
        bond_feats3 = self.noncov_graph(bg3, atom_feats, bond_feats3)
        readouts = self.readout(bg3, bond_feats3)
        return self.FC(readouts)
